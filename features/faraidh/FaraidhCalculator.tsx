...
import { FARAIDH_FAQ } from './constants.ts';
import { FaraidhProvider, useFaraidh } from './context/FaraidhContext.tsx';
import { FaPen, FaChartPie, FaHistory } from 'react-icons/fa';

type FaraidhTab = 'input' | 'result' | 'history';

// Inner Component that consumes the context
const FaraidhContent: React.FC<{ 
    activeTab: FaraidhTab; 
    setActiveTab: (tab: FaraidhTab) => void 
}> = ({ activeTab, setActiveTab }) => {
    // Consume Context (No more passing props down!)
    const { 
        result, 
        deceasedGender, 
        history, 
        ...
        <div className="max-w-screen-2xl mx-auto pb-0 lg:pb-16 animate-fade-in px-0 md:px-6">
            ...
            <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 items-start mt-4 lg:mt-0 relative">
                <div className={`flex-1 w-full min-w-0 ${activeTab === 'input' ? 'block' : 'hidden lg:block'}`}>
                    {/* Components now consume context internally */}
                    <HeirsForm />
...
// Main Component - Wrapper for Provider
const FaraidhCalculator: React.FC = () => {
    const [activeTab, setActiveTab] = useState<FaraidhTab>('input');

    return (
        <FaraidhProvider setActiveTab={setActiveTab}>
            <FaraidhContent activeTab={activeTab} setActiveTab={setActiveTab} />
        </FaraidhProvider>
    );
};
