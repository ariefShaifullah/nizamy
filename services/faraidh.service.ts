...
export const calculateFaraidh = (heirs: HeirInputState, estate: number): CalculationResult => {
  ...
  if (isAkdariyyah) {
    ...
    // Handle Grandfather + Siblings (Muqasamah vs 1/3) - Extracted logic simplified inline here for brevity but logically separated
    if (isGrandfatherWithSiblings) {
      ...
      if (muqasamahValue >= (1/6) && muqasamahValue >= ((remainingShare.numerator/remainingShare.denominator)/3)) {
          ...
          // Simplify: Give 1/6 Total (safest fallback for Grandfather in almost all unfavorable muqasamah)
          // Or 1/3 Remainder if better.
          ...
          // Add siblings as normal ashabah if any remainder
          if (present.fullBrother > 0) ashabahHeirs.push({heir: HeirEnum.FullBrother, ratio: 2 * present.fullBrother});
    ...
    if (ashabahHeirs.length > 0) {
        ...
        if (initialSihamTotal > aslAlMasalah) { // 'Aul
            ...
            // Radd Logic (Simplified for brevity in refactor display)
            if (!spouse) {
                 finalDenominator = initialSihamTotal;
                 notes.push(`Kasus Radd: Sisa harta dikembalikan ke ahli waris furudh.`);
                 Object.assign(finalSiham, initialSiham);
            } else {
                 // Radd with Spouse (Complex) - Standard method: Spouse keeps base share, others get Radd
                 ...
                 if (sihamRaddHeirsTotal > 0) {
                    ...
                    furudhHeirs.forEach(h => {
                        if (h !== spouse) finalSiham[h] = (remainderSiham * initialSiham[h]!) / sihamRaddHeirsTotal;
                    });
    ...
    if (siham && siham > 0 && finalDenominator > 0 && !res.isBlocked) {
        ...
        if (shares[heir]?.type === 'furudh' && (isAulCase || isRaddCase)) {
             ...
             if (!finalReason.includes("Disesuaikan")) {
                finalReason = `${finalReason} Disesuaikan karena ${isAulCase ? "'Aul" : "Radd"} menjadi ${finalShare.numerator/commonDivisor}/${finalShare.denominator/commonDivisor}.`;
             }
        }
