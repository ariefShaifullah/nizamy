...
const FaraidhCalculator: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto pb-24 lg:pb-0 animate-fade-in">
        {/* Header - Hidden on Mobile (To match Zakat style layout) */}
        <div className="hidden lg:block text-center mb-8 lg:mb-12">
          <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight text-blue-900 dark:text-blue-400 sm:text-5xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start mt-4 lg:mt-0">
          ...
          <div className={`lg:col-span-1 ${activeTab === 'input' ? 'block' : 'hidden lg:block'}`}>
          ...
          <div className={`lg:col-span-2 ${activeTab === 'result' ? 'block' : 'hidden lg:block'}`}>
            <ResultsDisplay result={result} />
          </div>

          {/* Mobile Only: History Tab */}
          <div className={`lg:hidden ${activeTab === 'history' ? 'block' : 'hidden'}`}>
            <HistoryPanel history={history} onLoad={loadFromHistory} onClear={clearHistory} />
            {history.length === 0 && (
        ...
        <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 pb-safe px-2 py-2 z-50 flex justify-between items-center shadow-[0_-4px_10px_rgba(0,0,0,0.05)]">
             <button 
