...
export const calculateFaraidh = (heirs: HeirInputState, estate: number): CalculationResult => {
  ...
  if (totalHeirsCount === 0) {
    return {
      ...
      finalDenominator: 0,
      notes: [
        "Tidak ada ahli waris yang diketahui. Seluruh harta diserahkan ke Baitul Mal atau digunakan untuk kemaslahatan umum.",
      ],
    };
  }
  ...
  if (sonExists && (grandsonExists || granddaughterExists)) {
    notes.push(
      "INFO KHI: Cucu yang orang tuanya meninggal lebih dulu bisa menjadi 'Ahli Waris Pengganti' (Pasal 185 KHI)."
    );
  }
  ...
  if (isAkdariyyah) {
    ...
    if (isGrandfatherWithSiblings) {
      ...
      const muqasamahValue =
        (remainingShare.numerator / remainingShare.denominator) *
        (2 / totalHeads);
      ...
      if (
        ...
        else if (present.paternalBrother > 0)
          ashabahHeirs.push({
            heir: HeirEnum.PaternalBrother,
            ratio: 2 * present.paternalBrother,
          });
    ...
    if (ashabahHeirs.length > 0) {
      ...
      if (remainingSiham > 0) {
        ...
        ashabahHeirs.forEach((h) => {
          if (shares[h.heir]?.type !== "furudh" && results[h.heir]) {
            ...
            results[h.heir]!.reason =
              "Terhalang karena harta habis oleh ahli waris furudh.";
          }
        });
        ...
        if (initialSihamTotal > aslAlMasalah) {
          ...
          notes.push(
            `Kasus 'Aul: Asal masalah ${aslAlMasalah} meningkat menjadi ${finalDenominator}.`
          );
        } else {
      ...
      if (initialSihamTotal > aslAlMasalah) {
      ...
      } else if (initialSihamTotal < aslAlMasalah && initialSihamTotal > 0) {
        // Radd or Sisa
        const spouse =
          present.husband > 0
            ? HeirEnum.Husband
        ...
        if (!spouse) {
          ...
          finalDenominator = initialSihamTotal;
          notes.push(
            `Kasus Radd: Sisa harta dikembalikan ke ahli waris furudh secara proporsional.`
          );
          Object.assign(finalSiham, initialSiham);
        } else {
          ...
          if (sihamRaddHeirsTotal > 0) {
            ...
            notes.push(
              "Kasus Radd: Sisa harta dikembalikan kepada ahli waris furudh selain pasangan."
            );
          } else {
            // Spouse Only Case (Siham Radd Heirs = 0)
            // Spouse takes their share, remainder goes to Baitul Mal / Sisa
            finalDenominator = aslAlMasalah;
            Object.assign(finalSiham, initialSiham);
            notes.push(
              "Ahli waris hanya pasangan. Sisa harta diserahkan ke Baitul Mal (menurut Jumhur) atau Dzawil Arham."
            );
          }
        }
        totalSharesNum = finalDenominator;
      } else {
        // Adil (Sum == Asal Masalah)
        finalDenominator = aslAlMasalah;
        totalSharesNum = initialSihamTotal;
        Object.assign(finalSiham, initialSiham);
      }
  ...
  for (const key of Object.keys(results)) {
    ...
    let finalEvidence =
      res.evidence ||
      (res.isBlocked ? LEGAL_BASIS.HADITH_NEAREST_MALE : QURAN_REFS.AN_NISA_11);
    ...
    if (siham && siham > 0 && finalDenominator > 0 && !res.isBlocked) {
      ...
      if (shares[heir]?.type === "furudh" && (isAulCase || isRaddCase)) {
        ...
        if (!finalReason.includes("Disesuaikan")) {
          finalReason = `${finalReason} Disesuaikan karena ${
            isAulCase ? "'Aul" : "Radd"
          }.`;
        }
      }
      ...
      finalResults.push({
        ...res,
        share: { numerator: 0, denominator: 1, type: "none" },
        finalShare: { numerator: 0, denominator: 1, type: "none" },
        percentage: 0,
        value: 0,
        evidence: finalEvidence,
      } as HeirResult);
