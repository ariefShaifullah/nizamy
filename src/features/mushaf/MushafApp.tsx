...
import { FaArrowLeft, FaHashtag, FaQuestionCircle, FaCog, FaBookmark, FaEllipsisV } from 'react-icons/fa';
...
const MushafAppContent: React.FC = () => {
    ...
    // Tutorial Check
    useEffect(() => {
        const hasSeen = localStorage.getItem('nizamy_mushaf_tooltip_seen');
        if (!hasSeen && session) {
    ...
    const handleCloseOnboarding = () => {
        localStorage.setItem('nizamy_mushaf_tooltip_seen', 'true');
                ...
                <div className="flex justify-between items-center px-4 py-3 max-w-3xl mx-auto w-full">
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        <div className="relative" ref={menuRef}>
                            ...
                            >
                                <div className="icon-wrapper w-4 h-4"><FaEllipsisV /></div>
                            </button>

                            {isMenuOpen && (
                                <div className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-100 dark:border-slate-800 z-dropdown flex flex-col p-1 animate-fade-in-up origin-top-right">
                                    {session.type === 'surah' && (
                                        <button
                                            onClick={() => { setIsJumpModalOpen(true); setIsMenuOpen(false); }}
                                            className="w-full text-left px-4 py-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 flex items-center gap-3 text-sm font-medium text-slate-700 dark:text-slate-300 transition-colors"
                                        >
                                            <span className="text-slate-400"><FaHashtag /></span> Loncat Ayat
                                        </button>
                                    )}
                                    <button
                                        ...
                                        className="w-full text-left px-4 py-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 flex items-center gap-3 text-sm font-medium text-slate-700 dark:text-slate-300 transition-colors"
                                    >
                                        <span className="text-slate-400"><FaCog /></span> Pengaturan
                                    </button>
                                    <button
                                        onClick={() => { setIsHelpOpen(true); setIsMenuOpen(false); }}
                                        className="w-full text-left px-4 py-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 flex items-center gap-3 text-sm font-medium text-slate-700 dark:text-slate-300 transition-colors"
                                    >
                                        <span className="text-slate-400"><FaQuestionCircle /></span> Bantuan
                                    </button>
                                </div>
                            )}
                        </div>
