...
import { useSurahData, type SurahInfo } from "../../../hooks/useSurahData.ts";
...
// Local helper to filter surahs by target Juz (uses reactive data)
const filterSurahsByJuz = (surahs: SurahInfo[], targetJuz: number): SurahInfo[] => {
  if (targetJuz === 30) {
    return surahs.filter((s) => s.number >= 78 && s.number <= 114);
  }
  if (targetJuz === 29) {
    return surahs.filter((s) => s.number >= 67 && s.number <= 77);
  }
  return surahs;
};
...
export const AddItem: React.FC<AddItemProps> = ({
  ...
  const { surahs: allSurahs, isLoading: isSurahDataLoading } = useSurahData();
  ...
  // Use reactive surah data from hook
  const availableSurahs = useMemo(() => filterSurahsByJuz(allSurahs, profile.targetJuz), [allSurahs, profile.targetJuz]);
  const selectedSurahData = useMemo(() => allSurahs.find(s => s.number === selectedSurahNumber), [allSurahs, selectedSurahNumber]);
  ...
  useEffect(() => {
    // Wait for surah data to load before initializing
    ...
    if (lastAddedItem) {
      const surahRef = allSurahs.find(s => s.number === lastAddedItem.surahNo);
      ...
      if (surahRef) {
        if (lastAddedItem.endAyah < surahRef.verses) {
          ...
          if (profile.targetJuz === 114) {
            ...
            const available = filterSurahsByJuz(allSurahs, profile.targetJuz);
      ...
      // NEW USER: No existing items, initialize based on profile target
      const available = filterSurahsByJuz(allSurahs, profile.targetJuz);
      if (available.length > 0) {
        suggestedSurahNo = available[0].number;
      }
  ...
  }, [allSurahs, profile.targetJuz]);

  useEffect(() => {
    setQuotaWarning(null);
    const surah = allSurahs.find((s) => s.number === selectedSurahNumber);
    if (!surah) return;
  ...
  const handleSelectSurah = (surahNum: number) => {
    ...
    const surahRef = allSurahs.find(s => s.number === surahNum);
          ...
          <div className="space-y-6">
            {!hasPending && (
              <div className="grid grid-cols-2 gap-3">
                <div
                  className={`text-center p-3 rounded-xl border ${dailyRemaining > 0
                ...
                <div
                  className={`text-center p-3 rounded-xl border ${isSelectionOverQuota
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  type="number"
                  className={`w-full border rounded-xl p-4 font-bold text-center outline-none focus:ring-2 bg-white dark:bg-slate-900 text-slate-900 dark:text-white ${inputError && inputError.includes("Ayat awal")
              ...
              {filteredSurahs.length === 0 ? (
                ...
                filteredSurahs.map((s) => (
                  <button
                    ...
                    className={`w-full flex items-center p-3 rounded-xl mb-1 transition-all ${selectedSurahNumber === s.number
                      ? "bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-100 dark:border-indigo-800"
                      : "hover:bg-slate-50 dark:hover:bg-slate-700/50 border border-transparent"
                      }`}
                  >
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold mr-4 ${selectedSurahNumber === s.number
