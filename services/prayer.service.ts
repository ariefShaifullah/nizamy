...
// Reverse Geocoding (Simple Approximation or via API if needed, keeping it simple for now)
// We will rely on API meta or generic name if reverse geocoding is too heavy.
// Aladhan API returns timezone, but not city name directly in timings endpoint.
// We will use a simple display logic.
...
export const getNextPrayer = (
  timings: PrayerTimes
): { name: string; time: string; diffMs: number; isTomorrow: boolean } => {
  const now = new Date();
